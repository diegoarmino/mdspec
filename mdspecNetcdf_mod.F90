...
! DEFINES
  ...
  character(11), parameter :: NCREMD_VALUES = "remd_values"
  character(10), parameter :: NCREMD_TYPES = "remd_types"
  ...
  public NC_create, NC_setupAmberNetcdf, NetcdfFileExists, NC_error, &
         ...
         NC_setupRemdDimension, NC_setupRemdTypes
...
!--------------------------------------------------------------------
!> MODULE AMBERNETCDF FUNCTION NC_SETUPREMDVALUES
!> @brief Look for replica remd_values info.
integer function NC_setupRemdValues(ncid)
  use netcdf
  implicit none
  integer, intent(in) :: ncid
  integer TempVID

  NC_setupRemdValues=-1
  if ( nf90_inq_varid(ncid,NCREMD_VALUES,TempVID) .eq. NF90_NOERR ) then
#   ifdef _NETCDF_DEBUG
    write(mdout,'(a)') 'Netcdf file has replica remd_values.'
#   endif
    NC_setupRemdValues = TempVID
  endif
end function NC_setupRemdValues

!--------------------------------------------------------------------
!> MODULE AMBERNETCDF FUNCTION NC_SETUPREMDDIMENSION
!> @brief Look for replica remd_dimension info.
integer function NC_setupRemdDimension(ncid, remd_dimension)
  use netcdf
  implicit none
  integer, intent(in) :: ncid
  integer, intent(out) :: remd_dimension
  integer RemdDimensionVID

  NC_setupRemdDimension=-1
  if ( nf90_inq_dimid(ncid, NCREMD_DIMENSION, RemdDimensionVID) .eq. NF90_NOERR ) then
    NC_setupRemdDimension = GetDimInfo(ncid, NCREMD_DIMENSION, remd_dimension)
  end if
end function NC_setupRemdDimension

!--------------------------------------------------------------------
!> MODULE AMBERNETCDF FUNCTION NC_SETUPREMDTYPES
!> @brief Look for replica remd_types info.
integer function NC_setupRemdTypes(ncid)
  use netcdf
  implicit none
  integer, intent(in) :: ncid
  integer RemdTypesVID

  NC_setupRemdTypes=-1
  if ( nf90_inq_varid(ncid,NCREMD_TYPES,RemdTypesVID) .eq. NF90_NOERR ) then
#   ifdef _NETCDF_DEBUG
    write(mdout,'(a)') 'Netcdf file has replica remd_types.'
#   endif
    NC_setupRemdTypes = RemdTypesVID
  endif
end function NC_setupRemdTypes
...
!> @param hasRemdValues If true, file will contain REMD values
...
logical function NC_create(filename, owrite, isRestart, natomIn, hasCoords, &
                           hasVelocity, hasBox, hasRemdValues, hasTime, &
                           hasFrc, title, &
...
!  character,    intent(in)  :: facc
  ...
  logical,      intent(in)  :: hasCoords, hasVelocity, hasBox, hasRemdValues
...
# ifdef _NETCDF_DEBUG
  write(mdout,'(a,a,a,i8,4(a,l1))') "| DEBUG: NC_create: ", trim(filename), &
        ...
        "  remd_values=", hasRemdValues, "  time=", hasTime
# endif
  ...
  ! Replica REMD Values
  if (hasRemdValues) then
    if (present(frameDID)) dimensionID(1) = frameDID;
    TempVID = NC_define_var(ncid,NCREMD_VALUES,NF90_DOUBLE,NDIM-2,dimensionID)
    if (TempVID .eq. -1) return !  'NetCDF error on defining remd_values'
  endif
  ! Set fill mode
...
logical function NC_setupMdcrd(ncid, title, nframes, ncatom, &
  ...
  ! Setup replica remd_values
  TempVID = NC_setupRemdValues( ncid )
  ! All is well
  NC_setupMdcrd=.false.
...
logical function NC_defineRemdIndices(ncid, remd_dimension, indicesVID, &
  ...
  integer, optional, intent(out)    :: remd_groupsVID, remd_typesVID
  integer, intent(out)              :: remd_valuesVID
  ! Local vars
  integer, dimension(NF90_MAX_DIMS) :: dimensionID
  ...
  if (isMREMD) then ! MREMD setup block
    ...
    if (isRestart) then
      indicesVID = NC_define_var(ncid, NCREMD_INDICES, NF90_INT, &
                                          1, dimensionID)
      remd_valuesVID = NC_define_var(ncid, NCREMD_VALUES, NF90_DOUBLE, &
                                          1, dimensionID)
      ...
      if (.not.present(remd_typesVID)) then
        write(mdout,'(a)') 'Internal Error: NC_defineRemdIndices called without remd_typesVID'
        return
      endif
      remd_typesVID = NC_define_var(ncid, NCREMD_TYPES, NF90_INT, 1, dimensionID)
    else
      if (.not.present(frameDID)) then
    ...
    if (remd_valuesVID.eq.-1) return ! 'define replica remd_values'
    ! Eventually store temperature tables for each REMD dimension
...
!!         -1 if error.
integer function NC_setupMultiD(ncid, expected_dim, indicesVID, groupsVID, valuesVID)
  use netcdf
  implicit none
  integer, intent(in)  :: ncid, expected_dim
  integer, intent(out) :: indicesVID, groupsVID, valuesVID
  ! local
  ...
  if ( NC_error(nf90_inq_varid(ncid, NCREMD_VALUES, valuesVID),&
                'Getting values VID') ) return
  ! All is well
  NC_setupMultiD=0
...
# endif
  ...
  ! local
  integer ncid, indicesVID, groupsVID, valuesVID, repidxVID, crdidxVID, ierr
...
# ifdef BINTRAJ
  ...
  ierr = NC_setupMultiD(ncid, remd_dimension, indicesVID, groupsVID, valuesVID)
  if (ierr.eq.0) then
    ! Get indices
